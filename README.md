...
Ensure Cargo's bin directory (normally `~/.cargo/bin`) is on `PATH`. For use
...
cycle the board, cable, JTAG index, and HDL language defaults.
...
Every top-level command accepts a one-letter alias: `n` (new), `i` (init),
...
cable name is preferable to a number when multiple cables are attached. It always
runs the build flow first and stops on failure or a missing `.sof`. This command
configures FPGA SRAM; persistent flash programming and `.pof` operations are not
...
and refusing stale/missing programming output. Hardware programming is tested
